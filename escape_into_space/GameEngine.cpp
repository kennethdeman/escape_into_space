...
int HitOutline::Raycast(DOUBLE2 p, DOUBLE2 v, double maxLambda, HIT* hitArr, int hitArrSize, double sense)
{
	...
	if( lineVertexCount > 0 )
	{
		...
		RECT2 r1,r2;//not inside the for-loop for performance reasons
		for(int count = 0; count < lineVertexCount && hitCount < MAX_HITS; count += 2 )
		{
			...
			r1.left=min(p1.x,p2.x);
			r1.top= min(p1.y,p2.y);
			r1.right= max(p1.x,p2.x);
			r1.bottom= max(p1.y,p2.y);
			r2.left=min(q1.x,q2.x);
			r2.top=min(q1.y,q2.y);
			r2.right=max(q1.x,q2.x);
			r2.bottom=max(q1.y,q2.y);
	...
	std::sort(m_HitArr, m_HitArr+hitCount, sortHitOnTime);

	hitCount = min(hitCount, hitArrSize);
	for(int count = 0; count < hitCount; ++count )
	{
		hitArr[count]=m_HitArr[count];
	}
	return hitCount;
}
